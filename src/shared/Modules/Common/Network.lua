...
if IsServer then
	...
	Communication = ReplicatedStorage:WaitForChild("Network")
...
if IsServer then
	...
	function Network:GetPlayers(): Array<Player>
	...
	function Network:GetPlayerPosition(player): Vector3 | nil
    ...
    --[[
        ...
        @example
            ...
            Network:FireOtherClientsWithinDistance(game.Players.Player1, 100, 'MyEvent', 'Hello')
    ]]
    ...
    --[[
        ...
        Network:FireAllClientsWithinDistance(Vector3.new(0, 0, 0), 100, 'MyEvent', 'Hello')
    ]]
	...
	--[[
        Invokes an event on the provided client and waits for a response with a specified timeout in seconds.
        Catches if the invocationt imed out or the handler errored.

        @example

            Network:InvokeClientWithTimeout(10, game.Players.Player1, 'MyEvent', 'Hello'):andThen(function(result)
				print(result)
			end):catch(function()
				print("The invocation timed out or errored!")
			end)
	...
	function Network:InvokeClientWithTimeoutPromise(timeout, client, name, ...)
		local parameters = { ... }
		return Promise.new(function(resolve, reject)
			local _, result = pcall(function()
				return { self:InvokeClientWithTimeout(timeout, client, name, unpack(parameters)) }
			end)
			...
			if isSuccess then
				...
				reject(if typeof(result) == 'string' then result else unpack(result))
			end
		end)
	end
	...
	--[[
		--[[
        Invokes an event on the provided client and waits for a response.
        Catches if invocation timed out or the handler errored.

        @example

            Network:InvokeClient(game.Players.Player1, 'MyEvent', 'Hello'):andThen(function(result)
				print(result)
			end):catch(function()
				print("The invocation timed out or errored!")
			end)
	]]
	function Network:InvokeClientPromise(...)
		return self:InvokeClientWithTimeoutPromise(60, ...)
	end
    --[[
	...
	--[[
        ...
        Promise catches if the invocation timed out or the handler errored.

        @example

            local returned = Network:InvokeServer(10, "MyFunction1", "Hello"):andThen(function(returned)
				print('MyFunction1 returned: ', returned)
			end):catch(function(returned)
				warn('MyFunction1 errored: ', returned)
			end)
    ]]
	function Network:InvokeServerWithTimeoutPromise(timeout, name, ...)
		local parameters = {...}
		return Promise.new(function(resolve, reject)
			local success, result = pcall(function()
				return { self:InvokeServerWithTimeout(timeout, name, table.unpack(parameters)) }
			end)
			if success then
				resolve(table.unpack(result))
			else
				reject(if typeof(result) == 'string' then result else table.unpack(result))
			end
		end)
	end
	...
	--[[
        Invokes the RemoteFunction attached to name, with the given parameters, but returns a Promise

        @example

            local returned = Network:InvokeServer("MyFunction1", "Hello"):andThen(function(returned)
				print('MyFunction1 returned: ', returned)
			end):catch(function(returned)
				warn('MyFunction1 errored: ', returned)
			end)
    ]]
	function Network:InvokeServerPromise(name, ...)
		return Network:InvokeServerWithTimeoutPromise(nil, name, ...)
	end
end
...
do
	...
	function Network:AddReferenceAlias(key, refType, ...)
		...
		if not refData then
			warn("[Network] Tried to add an alias to a non-existing reference")
			return
		end
	...
	function Network:RemoveReference(key, refType)
		...
		local refData = References[refType][key]
		if not refData then
			warn("[Network] Tried to remove a non-existing reference")
			return
		end
